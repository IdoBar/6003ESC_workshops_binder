...
title: "Tutorial for Workshops 1-2 - Introduction to Visualisation and Data Analysis"
author: "Dr. Ido Bar"
date: "06/03/2023"
output: 
...
pacman::p_load(htmltools, knitr, tidyverse, fontawesome, 
               here, esquisse)
# pacman::p_install_gh("gadenbuie/tweetrmd",
#                       "hadley/emo", update = FALSE)
...
Using Binder is as simple as clicking on the Binder badge - [![Launch Rstudio Binder](../figs/binder_logo.png){width=125px}](https://mybinder.org/v2/gh/IdoBar/6003ESC_workshops_binder/T1_2023?urlpath=rstudio){target="_blank"}.  
...
Installing new packages in R can sometimes be quite challenging and time-consuming using the general approach described below, however, I recently found an auxiliary package called [pak](https://pak.r-lib.org/){target="_blank"}, that makes installing other packages a breeze, so we will use it throughout these workshops.
...
We will use the `ggplot2` package, which stands for "Grammar of Graphics" and breaks up graphs into semantic components such as scales and layers that can be added to the plotting area, as can be seen in Figure \@ref(fig:ggplot-layers).  
...
```{r ggplot-layers, echo=FALSE, fig.cap="A visualisation of the layer concept in 'ggplot2' package (starting from bottom up, credit to [Coding Club](https://ourcodingclub.github.io/tutorials/dataviz-beautification-synthesis/#distributions)).", out.width = '80%'}
...
```{r expct-per-year1, include=TRUE, echo=TRUE, fig.cap="Life expectancy by years in Asian countries (first try).", out.width = '80%'}
...
We got an empty canvas in Figure \@ref(fig:expct-per-year1), but it had been sized to fit our range of data on the X and Y axes, that will be our first layer of the plot (like the bottom one in Figure \@ref(fig:ggplot-layers)).  
...
```{r expct-per-year2, include=TRUE, echo=TRUE, fig.cap="Life expectancy by years in Asian countries (added line graph).", out.width = '80%'}
# create the plot
ggplot(data = gap_asia, mapping = aes(x = year, y = lifeExp)) + 
  geom_line(size=1) 
...
What just happened (Figure \@ref(fig:expct-per-year2))? Can you guess?  
...
```{r expct-per-year3, include=TRUE, echo=TRUE, fig.cap="Life expectancy by years in Asian countries (added line graph coloured by country).", out.width = '80%'}
# create the plot
ggplot(data = gap_asia, mapping = aes(x = year, y = lifeExp, color=country)) + 
  geom_line(size=1) 
```

Hooray! this looks much better (Figure \@ref(fig:expct-per-year3)).  
...
The final result can be seen in Figure \@ref(fig:expct-per-year4).  

```{r expct-per-year4, include=TRUE, echo=TRUE, fig.cap="Life expectancy by years in Asian countries (beautify the plot with themes, colour palettes and labels).", out.width = '80%'}
ggplot(data = gap_asia, mapping = aes(x = year, y = lifeExp, color=country)) + 
...
We can use the `esquisser(viewer = "browser")` function to interactively create this plot (Figure \@ref(fig:esquisse)). 

```{r esquisse, echo=FALSE, fig.cap="Creating a ggplot interactively with esquisse package.", out.width = '85%'}
knitr::include_graphics(here("figs/esquisse_screenshot.png"))
```

Now copy the code from `esquisse` to the script file:

```{r gdp-per-expct1, include=TRUE, echo=TRUE, fig.cap="Relationship between GDP per capita and life expectancy by continent", out.width = '80%'}
# create plot
...
As we can see in Figure \@ref(fig:gdp-per-expct1), the markers overlay each other and it's hard understanding where there's a high density of data points and if they cover others behind them. To fix it, we'll make the points semi-transparent using `alpha=0.5` argument in `geom_point()`.  
...
Check out Figure \@ref(fig:gdp-per-expct2), the plot looks much better! 
We'll save this plot to the `output` folder as well.

```{r gdp-per-expct2, include=TRUE, echo=TRUE, fig.cap="Relationship between GDP per capita and life expectancy by continent (log-scaled X-axis).", out.width = '80%'}
# create plot
ggplot(data = gapminder, mapping = aes(x = gdpPercap, y = lifeExp, color=continent)) +
...
```{r chatgpt-tweet}
tweetrmd::tweet_embed("https://twitter.com/michelnivard/status/1607014731440885760")
```
