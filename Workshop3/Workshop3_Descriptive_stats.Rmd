---
title: "Tutorial for Workshop 3 - Descriptive Statistics"
...
pacman::p_load(tidyverse, ggpubr, paletteer, fontawesome, 
               here, plotly, skimr, esquisse)
# pacman::p_install_gh("gadenbuie/tweetrmd", update=FALSE)
pacman::p_load_gh("gadenbuie/tweetrmd", update=FALSE)
...
To run R and RStudio on Binder, click on this badge - [![Launch Rstudio Binder](../figs/binder_logo.png){width=125px}](https://mybinder.org/v2/gh/IdoBar/6003ESC_workshops_binder/T1_2023?urlpath=rstudio){target="_blank"}.  
Additional instructions (and screenshots) on using R, RStudio and Binder are available in [Workshop1 Tutorial on BB](https://lms.griffith.edu.au/courses/9122/files/2607207?wrap=1){target="_blank"}.  


## Analyse Data in R
### Install Extra Packages

For this analysis we will again use some packages from the [Tidyverse](https://www.tidyverse.org/). Please install the packages if you're getting error when running the `library()` command.  In addition we will use the `skimr` package to produce descriptive statistics and summaries of the data, `paletteer` for custom colour palettes and `plotly` to create interactive plots.  
To install these packages, we use the `pkg_install('package')` command (from package [pak](https://pak.r-lib.org/){target="_blank"}), please note that the package name need to be quoted and that we only need to perform it once (if we followed step 3b above, or every time we restart the server if we didn't), or when we want or need to update the package.  Once the package was installed, we can load its functions using the `library(package)` command. _Note that in this case we use the package name without quotes!_.  
...
library(pak)
pkg_install(c("skimr","plotly","paletteer")  
...
We can get a detailed look at the descriptive statistics per continent (including rate of missing data) using the `skim()` function:
