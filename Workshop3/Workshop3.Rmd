...
pacman::p_load(tidyverse,  paletteer, glue, scales, plotly, lubridate, patchwork, visdat)
...
Look at the vaccination rates compared to the European average:

```{r EU_hosp_delta}
# subset our original data to our countries + Europe (European average)
EU_hosp_data <- covid_data %>% filter(location %in% c(countries_subset, "Europe")) 
# Get the EU average vacc. rates
EU_vac_data <- covid_data %>% filter(location=="Europe") %>% select(date, EU_fully_vacc_percent=people_fully_vaccinated_per_hundred)
# create a new calculated column with the difference from the EU average
comp_vac_data <- EU_hosp_data %>% left_join(EU_vac_data) %>% 
  mutate(fully_vacc_change=people_fully_vaccinated_per_hundred-EU_fully_vacc_percent,
         location=fct_relevel(factor(location), "Europe", after = Inf)) %>% 
  filter(date>"2021-01-01") 

ggplot(comp_vac_data, 
             aes(x=date, y = fully_vacc_change/100 ,colour=location)) +
  geom_line(size=0.75, linetype="dashed") + 
  guides(color = guide_legend(override.aes = list(linetype="solid") ) ) +
  scale_y_continuous(labels=percent) + 
  scale_color_paletteer_d(col_pal) +
  scale_x_date(name = NULL,
               breaks = breaks_width("2 months"), 
               labels = label_date_short()) + 
  labs(color="Country", y = "Fully vaccinated (in comparison to the EU average)") +
  theme_bw(def_text_size) + 
  theme(panel.grid.minor = element_blank())
...
# save the plot to pdf file
ggsave("output/hospit_vacc_rates_EU_average.pdf", width=12, height = 8)
```
